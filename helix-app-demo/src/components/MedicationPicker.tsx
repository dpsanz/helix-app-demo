...
export default function MedicationPicker({ name, dosageMg, onChange }: Props) {
  ...
  return <div className="medication-picker">
    ...
    {open && matches.length > 0 && <div className="medication-results">{matches.map(item => <button type="button" key={item.name} onMouseDown={event => event.preventDefault()} onClick={() => { onChange(item.name, item.dosages[0]); setOpen(false) }}><span><strong>{item.name}</strong><small>{item.category}</small></span><em>{item.dosages[0]}–{item.dosages.at(-1)} mg</em></button>)}</div>}
